...
class LinkedList implements Iterator, Vector, Deque {
    ...
    /**
     * Returns the list flattened into a normal array.
     *
     * @return array The data from the list.
     */
    ...
    /**
     * Apply an arbitrary callback to each element of the list, returning the
     * same structure with the return values in place.
     *
     * @param Callable $callback The callback to apply: func($value, $key)
     * @return LinkedList The same structure, with values replaced.
     * @throws BadFunctionCallException when $callback is not callable.
     */
    public function map($callback) {
        ...
        //reference to internal list to save on method calls
        $listInternal = &$list->list;

        foreach($this->list as $key => $value) {
            $listInternal[] = $callback($value, $key);
        }
    ...
    /**
     * Apply an arbitrary callback to each element of the list, returning the
     * same structure with only items the callback returns true for
     *
     * @param Callable $callback The callback to apply: func($value, $key)
     * @return LinkedList The same structure, with filtered values only.
     * @throws BadFunctionCallException when $callback is not callable.
     */
